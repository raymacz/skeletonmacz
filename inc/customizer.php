 ...
 */
function skeletonmacz_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'theme_bg_color', 
				array(
					'label'		=> __( 'Header and footer background color', 'skeletonmacz'),
					'section'	=> 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'interactive_color', array(
				'label'		=> __( 'Interactive color (links etc)', 'skeletonmacz' ),
				'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options', 
		array(
			'title'			=> __( 'Theme Options', 'skeletonmacz' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'skeletonmacz' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'skeletonmacz_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);

	// Add the controls
	$wp_customize->add_control(	'skeletonmacz_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Index/archive displays', 'skeletonmacz' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Excerpt (default)', 'skeletonmacz' ),
				'full-content'	=> __( 'Full content', 'skeletonmacz' )
			),
...
add_action( 'customize_register', 'skeletonmacz_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function skeletonmacz_customize_preview_js() {
	wp_enqueue_script( 'skeletonmacz_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'skeletonmacz_customize_preview_js' );
...
function skeletonmacz_sanitize_length( $value ) {
...
if ( ! function_exists( 'skeletonmacz_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see skeletonmacz_custom_header_setup().
 */
function skeletonmacz_header_style() {
